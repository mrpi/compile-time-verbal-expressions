...
struct pattern
{
  impl::static_string<Len> str;

  constexpr pattern() {}

  template <size_t StrLen>
  constexpr pattern(const impl::static_string<StrLen>& str) : str(str)
  {
  }

private:
  template <size_t Len2>
  [[nodiscard]] constexpr auto add(impl::static_string<Len2> val) const
  {
...
template <size_t StrLen>
pattern(const impl::static_string<StrLen>& str)->pattern<StrLen>;
...
namespace impl
{
  ...
  constexpr auto sanitized_str_or_pattern(const char (&prefix)[BufLen1],
  ...
  template <size_t BufLen1, size_t BufLen2>
  constexpr auto sanitized_str(const char (&prefix)[BufLen1],
                               const char (&suffix)[BufLen2])
  {
    return [prefix, suffix](auto&& buf) {
      static_assert(!std::is_same_v<range, std::decay_t<decltype(buf)>>,
...
static inline constexpr auto word = pattern{impl::static_string{"\\w+"}};
static inline constexpr auto line_break =
    pattern{impl::static_string{"(?:\\r\\n|\\r|\\n)"}};
static inline constexpr auto br = line_break;
static inline constexpr auto something = pattern{impl::static_string{"(?:.+)"}};
static inline constexpr auto anything = pattern{impl::static_string{"(?:.*)"}};
